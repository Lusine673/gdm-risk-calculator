...
<style> :root{ --primary:#0ea5a2; /* бирюзовый акцент */ --ok:#1b5e20; /* зелёный текст */ --okbg:#e8f5e9; /* зелёный фон */ --warn:#b71c1c; /* красный текст */ --warnbg:#ffebee; /* красный фон */ --card:#ffffff; /* фон карточек */ --border:#e9eef2; /* светлая рамка */ } .block-container{padding-top:2rem;padding-bottom:2rem;} h3{font-weight:800;letter-spacing:.2px;margin-bottom:.8rem;} /* карточка ввода */ .card{ background:var(--card); border:1px solid var(--border); border-radius:14px; padding:16px 18px; box-shadow:0 2px 10px rgba(0,0,0,.04); } /* кнопка */ div.stButton > button{ background:linear-gradient(90deg,var(--primary),#14b8a6); color:#fff;border:0;border-radius:10px;padding:.65rem 1.05rem;font-weight:600; } div.stButton > button:hover{filter:brightness(1.05);} /* индикаторы */ .risk-high{background:var(--warnbg);color:var(--warn);padding:14px;border-radius:12px; text-align:center;font-size:20px;font-weight:800;} .risk-low{background:var(--okbg);color:var(--ok);padding:14px;border-radius:12px; text-align:center;font-size:20px;font-weight:800;} .hr{height:1px;background:#edf2f7;margin:1.25rem 0;} </style>
